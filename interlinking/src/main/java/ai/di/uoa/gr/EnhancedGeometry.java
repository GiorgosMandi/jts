...
public class EnhancedGeometry {
    ...
    List <Pair<int[], int[]>> edgesLabels = new ArrayList<>();
    List <Pair<int[], int[]>> nodesLabels = new ArrayList<>();


    EnhancedGeometry(Geometry geom, int index, LineIntersector li){
        argIndex = index;
        geometryGraph = new GeometryGraph(index, geom);
        geometryGraph.computeSelfNodes(li, false);
